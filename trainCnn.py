...
def train_cnn():
    ...
    # 读取数据集
    path = 'cnn_datasets\\'  # 车牌号数据集路径(车牌图片宽240，高80)
...
def cnn_predict(cnn, Lic_img):
    ...
    Lic_prediction = []
    for lic in Lic_img:
        lic_prediction = cnn.predict(lic.reshape(1, 80, 240, 3))  # 预测形状应为(1,80,240,3)
        lic_prediction = np.array(lic_prediction).reshape(7, 65)  # 列表转为ndarray，形状为(7,65)
        lic_prediction_int = lic.astype(np.uint8)  # 将lic_prediction类型转为int型
        # cv2.imshow("plate", lic_prediction_int)
